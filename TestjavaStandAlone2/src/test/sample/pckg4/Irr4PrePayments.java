...
public class Irr4PrePayments {

	
	public static void main(String args[]) {
		
		Irr4PrePayments irr= new Irr4PrePayments();
		
		String finalSolution = "";
		
		double amount = 1000000.00;
		double installment = 32480.45;
		int period = 48;
		int noOfPrePayments = 1;
		int repaymentFrequency = 12;
		
		System.out.println(" ");
		
		
		finalSolution = irr.calculateIrr(BigDecimal.valueOf(amount),BigDecimal.valueOf(installment),period,repaymentFrequency,noOfPrePayments);
	...
	public String calculateIrr(BigDecimal loanAmount,BigDecimal installment, int loanPeriod , int repaymentFrequency , int noOfPrePayments) {
		...
		BigDecimal npv = calculateNpv(installment,1,period,initialGuessIrr);
...
//			calculateUpToTwoDigit = iteration(loanAmount,installment,loanPeriod,calculateUpToNoneDigit,4);
			...
			while(currentIteration <= noOfIterations) {
				...
				guess = iteration(amount,installment,period,guess,currentIteration , repaymentFrequency);				
				currentIteration +=1;

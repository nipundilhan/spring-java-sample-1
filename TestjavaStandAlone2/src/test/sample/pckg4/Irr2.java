...
public class Irr2 {
	...
	public static void main(String args[]) {
		...
		System.out.println(" amount = 1020000.00 | installment =32601.84 | period = 48 |--------------- | irr = " +finalSolution);
				
		finalSolution = irr.calculateIrr(BigDecimal.valueOf(1520000.00),BigDecimal.valueOf(48583.13),48);
		System.out.println(" amount = 1520000 | installment =48583.13 | period = 48 |--------------- | irr = " +finalSolution);
		
		finalSolution = irr.calculateIrr(BigDecimal.valueOf(1520000.00),BigDecimal.valueOf(55043.46),48);
		System.out.println(" amount = 1520000.00 | installment =55043.46 | period = 48 |--------------- | irr = " +finalSolution);
		
		finalSolution = irr.calculateIrr(BigDecimal.valueOf(2520000.00),BigDecimal.valueOf(75255.84),60);
		System.out.println(" amount = 2520000.00 | installment =75255.84 | period = 60 |--------------- | irr = " +finalSolution);
...
/*			
			...
			System.out.println("************************************************* ");
			System.out.println(" ");
			System.out.println("--start up to four digit --------------------------");
			System.out.println(" ");
*/
			
			calculateUpToTwoDigit = iteration5(loanAmount,installment,loanPeriod,calculateUpToTwoDigit);
			
/*			
			System.out.println(" ");
			System.out.println(" ");
			System.out.println("--end up to four digit --------------------------");
*/			
		}
...
//			System.out.println(i.setScale(5, RoundingMode.FLOOR) + " -> r% value is - "+ (i.setScale(4, RoundingMode.FLOOR)).multiply(BigDecimal.valueOf(1200)) + " | npv is "+ tempNpv.setScale(2, RoundingMode.FLOOR));
	...
	public BigDecimal iteration5(BigDecimal loanAmount,BigDecimal installment, int loanPeriod , BigDecimal guess) {
		...
		BigDecimal limit = i.add(BigDecimal.valueOf(0.01));
		while ((limit).compareTo(i) >= 0) {
			BigDecimal tempNpv = calculateNpv(installment, 1, loanPeriod, i);
...
//			System.out.println(i.setScale(6, RoundingMode.FLOOR) + " -> r% value is - "+ (i.setScale(6, RoundingMode.FLOOR)).multiply(BigDecimal.valueOf(1200)) + " | npv is "+ tempNpv.setScale(2, RoundingMode.FLOOR));
...
//				
			if (loanAmount.compareTo(tempNpv) >= 0) {
				i = i.subtract(BigDecimal.valueOf(0.000001));
				finalAnswer = i.multiply(BigDecimal.valueOf(1200));
				finalAnswer = finalAnswer.setScale(2, RoundingMode.CEILING);
				break;
			}

			i = i.add(BigDecimal.valueOf(0.000001));
		}
